...
#ifndef OPENFLIGHTCONTROLLER_PWM_H
#define OPENFLIGHTCONTROLLER_PWM_H
...
/* Timer clock frequency [Hz] */
#define PWM_MAX_FREQ_HZ           100000000  /* 1MHz */   //TODO: change the max freq, it's not the right one!
...
void pwm_init(void) {
  ...
  /* set prescaler */
  timer_set_prescaler(TIM2, ((rcc_apb1_frequency * 2) / TIM_CLOCK_FREQ_HZ));
  /* enable preload */
  ...
  timer_set_period(TIM2, current_timer_cnt_period);
...
void pwm_set_dc(uint8_t ch_index, uint16_t dc_value_permillage) {
  ...
  if (dc_value_permillage <= 1000) {
    ...
    switch (ch_index) {
      case 0: {
        timer_set_oc_value(TIM2, TIM_OC1, dc_tmr_reg_value);
        break;
      }
      case 1: {
        timer_set_oc_value(TIM2, TIM_OC2, dc_tmr_reg_value);
        break;
      }
      case 2: {
        timer_set_oc_value(TIM2, TIM_OC3, dc_tmr_reg_value);
        break;
      }
      case 3: {
        timer_set_oc_value(TIM2, TIM_OC4, dc_tmr_reg_value);
        break;
      }
      default: {
...
#endif //OPENFLIGHTCONTROLLER_PWM_H
